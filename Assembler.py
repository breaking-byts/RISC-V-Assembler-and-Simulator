class Assembler:
   ...
   def I_type(self,instruction):
       if len(instruction)!=3 and len(instruction)!=4:
           raise SyntaxError(f"Line {self.get_line_number()}: Invalid number of arguments for I-type instruction: {instruction}")
   ...
   def R_type(self,instruction):
           if len(instruction)!=4:
               raise SyntaxError(f"Line {self.get_line_number()}: Invalid number of arguments for R-type instruction: {instruction}")
           opcode=self.opcodes[instruction[0]]
           rd=self.reg_to_binary(instruction[1])
           rs1=self.reg_to_binary(instruction[2])
   ...
   def U_type(self,instruction):
       if len(instruction)!=3:
           raise SyntaxError(f"Line {self.get_line_number()}: Invalid number of arguments for U-type instruction: {instruction}")
       opcode=self.opcodes[instruction[0]]
       rd=self.reg_to_binary(instruction[1])
       imm=self.get_immediate_binary(instruction[2],20)
   ...
   def S_type(self,instruction):
       if len(instruction)!=3:
           raise SyntaxError(f"Line {self.get_line_number()}: Invalid number of arguments for S-type instruction: {instruction}")
       opcode=self.opcodes[instruction[0]]
